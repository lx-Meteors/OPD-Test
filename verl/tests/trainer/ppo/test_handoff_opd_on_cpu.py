...
def test_select_best_verified_handoff_candidate_restores_parent_batch():
    # Parent 0 has three correct candidates; row 1 has the highest reward.
    # Parent 1 has no correct candidate and must fall back to canonical row 4.
    # Parent 2 is a non-handoff Student response.  Row 9 is an inactive dispatch
    # padding copy of parent 0 and must never be selected.
    ...
    # The selected correct candidate inherits parent 0's sole prefix OPD mask.
    assert selected.batch["handoff_opd_mask"][0].tolist() == [1, 1, 0, 0]
    # Parent 1 has no correct suffix, so its canonical suffix CE is disabled.
    assert selected.batch["handoff_ce_mask"][1].sum().item() == 0
    # The non-handoff row remains ordinary OPD and never acquires suffix CE.
