...
class RayPPOTrainer:
    ...
    def _init_prune_opd_dynamic_response_length(self):
        ...
        dynamic_cfg = prune_opd_cfg.get("dynamic_response_length", {}) if prune_opd_cfg is not None else {}
        self.prune_opd_dynamic_response_length_enabled = bool(dynamic_cfg.get("enable", False))
        self.prune_opd_dynamic_response_length_cfg = dynamic_cfg
        self.prune_opd_dynamic_response_length_no_hit_steps = 0

        static_max_len = int(self.config.actor_rollout_ref.rollout.response_length)
        max_len = min(int(dynamic_cfg.get("max_len", static_max_len)), static_max_len)
        min_len = int(dynamic_cfg.get("min_len", 1))
        if self.prune_opd_dynamic_response_length_enabled and min_len > max_len:
            raise ValueError(f"dynamic_response_length min_len={min_len} exceeds max_len={max_len}.")

        init_len = int(dynamic_cfg.get("init_len", max_len))
        self.prune_opd_dynamic_response_length_min_len = min_len
        self.prune_opd_dynamic_response_length_max_len = max_len
    ...
    def _update_prune_opd_dynamic_response_length(self, effective_response_length: torch.Tensor) -> dict[str, float]:
        ...
        cfg = self.prune_opd_dynamic_response_length_cfg
        current_len = int(self.prune_opd_dynamic_response_length)
        step = int(cfg.get("step", 100))
        hit_ratio_threshold = float(cfg.get("hit_ratio", 0.1))
        margin = int(cfg.get("margin", 0))
        shrink_patience = int(cfg.get("shrink_patience", 3))

        hit_boundary = max(self.prune_opd_dynamic_response_length_min_len, current_len - margin)
        hit_ratio = (effective_response_length >= hit_boundary).float().mean().item()

        next_len = current_len
        if hit_ratio >= hit_ratio_threshold:
            next_len = min(current_len + step, self.prune_opd_dynamic_response_length_max_len)
            self.prune_opd_dynamic_response_length_no_hit_steps = 0
        else:
            self.prune_opd_dynamic_response_length_no_hit_steps += 1
            if self.prune_opd_dynamic_response_length_no_hit_steps >= shrink_patience:
                next_len = max(current_len - step, self.prune_opd_dynamic_response_length_min_len)
                self.prune_opd_dynamic_response_length_no_hit_steps = 0

        self.prune_opd_dynamic_response_length = next_len
        return {
            "prune_opd/current_max_response_length": float(current_len),
            "prune_opd/next_max_response_length": float(next_len),
            "prune_opd/effective_response_length_hit_ratio": float(hit_ratio),
    ...
    def _log_prune_opd_loss_weights(self, batch: DataProto):
        ...
        try:
            ...
            use_wandb = "wandb" in self.config.trainer.logger
            use_swanlab = "swanlab" in self.config.trainer.logger and not use_wandb
            ...
            loss_weights = batch.batch["prune_opd_loss_weights"].detach().float()
            response_mask = batch.batch["response_mask"].detach().float()
            valid_counts = response_mask.sum(dim=0)
            valid_denom = valid_counts + 1e-6
            avg_loss_weights = (loss_weights * response_mask).sum(dim=0) / valid_denom
            avg_loss_weights_np = avg_loss_weights.cpu().numpy()

            if use_wandb or use_swanlab:
                plt.figure(figsize=(10, 6))
                plt.plot(
                    avg_loss_weights_np,
                    label=f"mean: {avg_loss_weights_np.mean():.4f}",
                    color="tab:purple",
                )
                plt.title(f"Prune-OPD Avg Loss Weight by Position (Step {self.global_steps})")
                plt.xlabel("Position")
                plt.ylabel("Avg Loss Weight")
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                ...
                if use_wandb:
                    plot = wandb.Image(
                        plt,
                        caption=f"Prune-OPD Avg Loss Weight by Position (Step {self.global_steps})",
                    )
                    wandb.log({"viz/prune_opd_loss_weight_mean": plot}, step=self.global_steps)
                else:
                    plot = swanlab.Image(
                        plt,
                        caption=f"Prune-OPD Avg Loss Weight by Position (Step {self.global_steps})",
                    )
                    swanlab.log({"viz/prune_opd_loss_weight_mean": plot}, step=self.global_steps)
                plt.close()

            valid_rows = torch.nonzero(response_mask.sum(dim=-1) > 0, as_tuple=False).flatten()
            if valid_rows.numel() == 0:
                return

            sample_offset = torch.randint(valid_rows.numel(), (1,), device=valid_rows.device).item()
            sample_idx = valid_rows[sample_offset].item()
            sample_loss_weights = loss_weights[sample_idx].cpu().tolist()
            sample_mask = response_mask[sample_idx].cpu().tolist()

            raw_weights = None
            if "prune_opd_weights" in batch.batch.keys():
                raw_weights = batch.batch["prune_opd_weights"][sample_idx].detach().float().cpu().tolist()

            output_dir = os.path.join("loss_weight", self.config.trainer.experiment_name)
            os.makedirs(output_dir, exist_ok=True)
            avg_output_path = os.path.join(output_dir, f"avg_step{self.global_steps}.csv")
            output_path = os.path.join(output_dir, f"step{self.global_steps}.csv")

            avg_loss_weights_list = avg_loss_weights.cpu().tolist()
            valid_counts_list = valid_counts.cpu().tolist()
            with open(avg_output_path, "w", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=["position", "avg_loss_weight", "valid_count"])
                writer.writeheader()
                for position, avg_loss_weight in enumerate(avg_loss_weights_list):
                    writer.writerow(
                        {
                            "position": position,
                            "avg_loss_weight": avg_loss_weight,
                            "valid_count": valid_counts_list[position],
                        }
                    )

            fieldnames = ["position", "loss_weight", "response_mask"]
            if raw_weights is not None:
                fieldnames.append("raw_weight")

            with open(output_path, "w", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                writer.writeheader()
                for position, loss_weight in enumerate(sample_loss_weights):
                    row = {
                        "position": position,
                        "loss_weight": loss_weight,
                        "response_mask": sample_mask[position],
                    }
                    if raw_weights is not None:
                        row["raw_weight"] = raw_weights[position]
                    writer.writerow(row)
        except Exception as e:
            print(f"Error logging prune-opd loss weights: {e}")
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.use_rm and "rm_scores" not in batch.batch.keys():
                            ...
                            batch.meta_info["opd_neg_kappa"] = self.config.actor_rollout_ref.rollout.get(
                                "opd_neg_kappa", 0.0
                            )
                        ...
                        if (self.global_steps == 1 or self.global_steps % 10 == 0) and "student_valid_counts" in batch.batch.keys():
                            try:
                                ...
                                if use_wandb:
                                    ...
                                    raise RuntimeError("No image logger configured for candidate plots.")
                                ...
                                if use_wandb:
                                    count_plot = wandb.Image(plt, caption=f"Candidate Counts (Step {self.global_steps})")
                                else:
                                ...
                                def make_line_plot(data, title, ylabel, caption, color="tab:blue", ylim=None):
                                    plt.figure(figsize=(10, 6))
                                    plt.plot(data, label=f"mean: {data.mean():.4f}", color=color)
                                    plt.title(title)
                                    plt.xlabel("Position")
                                    plt.ylabel(ylabel)
                                    if ylim is not None:
                                    ...
                                    if use_wandb:
                                        plot = wandb.Image(plt, caption=caption)
                                    else:
                                        plot = swanlab.Image(plt, caption=caption)
                                    plt.close()
                                    return plot
                                ...
                                if "Overlap" in plot_data and "Student" in plot_data and "Teacher" in plot_data:
                                    ...
                                    if use_wandb:
                                        ratio_student_plot = wandb.Image(
                                            plt, caption=f"Overlap / Student Ratio (Step {self.global_steps})"
                                        )
                                    else:
                                        ratio_student_plot = swanlab.Image(
                                            plt, caption=f"Overlap / Student Ratio (Step {self.global_steps})"
                                        )
                                    plt.close()
                                    ...
                                    if use_wandb:
                                        ratio_teacher_plot = wandb.Image(
                                            plt, caption=f"Overlap / Teacher Ratio (Step {self.global_steps})"
                                        )
                                    else:
                                        ratio_teacher_plot = swanlab.Image(
                                            plt, caption=f"Overlap / Teacher Ratio (Step {self.global_steps})"
                                        )
                                    plt.close()
                                ...
                                if (
                                    "responses" in batch.batch.keys()
                                    and "teacher_top_k_ids" in batch.batch.keys()
                                    and "teacher_top_k_log_probs" in batch.batch.keys()
                                ):
                                    responses = batch.batch["responses"]
                                    teacher_top_k_ids = batch.batch["teacher_top_k_ids"]
                                    teacher_top_k_log_probs = batch.batch["teacher_top_k_log_probs"]
                                    ...
                                    rank_idx = torch.arange(
                                        teacher_top_k_ids.size(-1), device=teacher_top_k_ids.device
                                    ).view(1, 1, -1)
                                    ...
                                    for threshold_label, threshold in [
                                        ...
                                        accept_mask = torch.where(
                                            has_crossed, top_p_mask, torch.ones_like(top_p_mask, dtype=torch.bool)
                                        )
                                        hit = ((teacher_top_k_ids == responses.unsqueeze(-1)) & accept_mask).any(dim=-1)
                                        hit_ratio = (hit.float() * response_mask_float).sum(dim=0) / valid_denom
                                        hit_ratio_np = hit_ratio.detach().cpu().numpy()
                                        log_payload[f"viz/hit_top_{threshold_label}_ratio"] = make_line_plot(
                                            hit_ratio_np,
                                            f"Student Token Hit Teacher Top-p Ratio p={threshold_label} (Step {self.global_steps})",
                                            "Hit Ratio",
                                            f"Hit Top-{threshold_label} Ratio (Step {self.global_steps})",
                                            color="tab:green",
                                            ylim=(-0.05, 1.05),
                                        )

                                if "old_log_probs" in batch.batch.keys() and "teacher_log_probs" in batch.batch.keys():
                                    student_probs = batch.batch["old_log_probs"].float().exp()
                                    teacher_probs = batch.batch["teacher_log_probs"].float().exp()
                                    token_prob_gap_abs = (student_probs - teacher_probs).abs()
                                    avg_token_prob_gap_abs = (
                                        token_prob_gap_abs * response_mask.float()
                                    ).sum(dim=0) / valid_denom
                                    log_payload["viz/token_prob_gap_abs"] = make_line_plot(
                                        avg_token_prob_gap_abs.detach().cpu().numpy(),
                                        f"Abs Student-Teacher Actual Token Prob Gap (Step {self.global_steps})",
                                        "Abs Prob Gap",
                                        f"Token Prob Gap Abs (Step {self.global_steps})",
                                        color="tab:red",
                                        ylim=(-0.05, 1.05),
                                    )

                                if use_wandb:
                                    wandb.log(log_payload, step=self.global_steps)
                                    print(f"Logged candidate plots to wandb at step {self.global_steps}")
                                else:
                    ...
                    if self.config.trainer.get("is_plot", False) and (self.global_steps == 1 or self.global_steps % 10 == 0):
                        try:
                            ...
                            if use_wandb:
                                ...
                                raise RuntimeError("No image logger configured for diagnostic plots.")
                            ...
                            if "teacher_entropy" in batch.batch.keys():
                                ...
                                plt.tight_layout()
                                if use_wandb:
                                    entropy_plot = wandb.Image(
                                        plt, caption=f"Teacher Entropy vs Position (Step {self.global_steps})"
                                    )
                                else:
                                    entropy_plot = swanlab.Image(
                                        plt, caption=f"Teacher Entropy vs Position (Step {self.global_steps})"
                                    )
                                plt.close()
                                ...
                                plt.tight_layout()
                                if use_wandb:
                                    adv_plot = wandb.Image(
                                        plt, caption=f"Advantage vs Position (Step {self.global_steps})"
                                    )
                                else:
                                    adv_plot = swanlab.Image(
                                        plt, caption=f"Advantage vs Position (Step {self.global_steps})"
                                    )
                                plt.close()
                                ...
                                plt.tight_layout()
                                if use_wandb:
                                    avg_entropy_plot = wandb.Image(
                                        plt, caption=f"Avg Teacher Entropy vs Position (Step {self.global_steps})"
                                    )
                                else:
                                    avg_entropy_plot = swanlab.Image(
                                        plt, caption=f"Avg Teacher Entropy vs Position (Step {self.global_steps})"
                                    )
                                plt.close()
                                ...
                                plt.tight_layout()
                                if use_wandb:
                                    avg_adv_plot = wandb.Image(
                                        plt, caption=f"Avg Advantage vs Position (Step {self.global_steps})"
                                    )
                                else:
                                    avg_adv_plot = swanlab.Image(
                                        plt, caption=f"Avg Advantage vs Position (Step {self.global_steps})"
                                    )
                                plt.close()
                                ...
                                if use_wandb:
                                    wandb.log(log_payload, step=self.global_steps)
                                    print(f"Logged 4 plots to wandb at step {self.global_steps}.")
                                else:
                                    swanlab.log(log_payload, step=self.global_steps)
                ...
                if "prune_opd_effective_response_length" in batch.batch:
                    effective_response_length = batch.batch["prune_opd_effective_response_length"].float()
                    metrics["prune_opd/effective_response_length_mean"] = effective_response_length.mean().item()
                    metrics["prune_opd/effective_response_length_max"] = effective_response_length.max().item()
