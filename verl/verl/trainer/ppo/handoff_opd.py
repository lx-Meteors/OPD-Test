...
def select_best_verified_handoff_candidate(
    ...
    """Keep at most one verified Teacher suffix for each Student rollout.

    All sampled Teacher suffixes are scored first.  For every Student prefix,
    the highest-scoring correct suffix is retained.  If none is correct, the
    canonical Student row is retained with suffix CE disabled.  The prefix OPD
    mask is moved to the selected row when necessary, so each Student rollout
    still contributes exactly one copy of prefix supervision.
    """
    ...
    for parent_index in torch.unique(parent_indices, sorted=True):
        ...
        # Exactly one real row owns prefix OPD.  Other candidate rows and
        # dispatch-padding rows have an all-zero prefix mask.
        ...
        if handoff_mask[group_rows].any():
