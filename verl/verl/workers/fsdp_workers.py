...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def teacher_ctx_spans(resp_len, window, segment):
        """Read-out spans for the windowed teacher, as ``(start, end, ctx_start)``.

        Truncation only bites once a span starts past ``window``, so every span up to
        and including the one starting at ``window`` carries ``ctx_start == 0`` and is
        the untruncated prefix. Those are emitted as one span rather than two: the
        teacher input ``prompt + response[0 : window + segment]`` is elementwise
        identical either way, so the read-out is bit-exact while costing one pass
        fewer. This matters most when ``window + segment >= resp_len``, where the
        intervention is a no-op and the merged form correctly collapses to a single
        plain pass.
        """
        onset = window + segment
        spans, cur = [], min(onset, resp_len)
        if cur > 0:
    ...
    def _teacher_forward_windowed(
        ...
        For a read-out span starting at ``s`` the teacher conditions on

            [prompt] + response[max(0, s - window) : s] + response[s : s + segment]

        and the read-out is kept only for the span. The student's trajectory is never
        modified -- the student still generates and scores under the full context; only
        the teacher's conditioning is truncated. Positions shallower than
        ``window + segment`` keep the whole prefix, so they are bit-exact with the
        plain path.

        This has to re-encode rather than mask, and therefore has to chunk. Masking the
        middle of a single fixed context is exactly equivalent to deleting it (verified:
        KL between the two is ~1e-4 nats), but a per-position band in one pass is a
        different operation entirely: over L layers a banded mask has a receptive field
        of L*(window-1), so far-back tokens still reach the read-out through the layer
        stack. Measured at window 4096 and depth >= 8192, banded attention moves
        KL(p||q) the wrong way, 0.778 -> 1.049, while re-encoding gives 0.742 -> 0.312.

        Because the context is amortized over a span, a position sees between ``window``
        and ``window + segment`` tokens of prefix. The slack is on the safe side (more
        context, closer to the untruncated teacher).
        """
    ...
    def compute_rm_score(self, data: DataProto, kl_estimator="k1"):
        ...
        with self.ulysses_sharding_manager:
            ...
            if ctx_window and ctx_window > 0:
                if self.rank == 0:
                    print(
                        f"Teacher context window ENABLED: window={ctx_window} segment={ctx_segment} "
                        f"-> truncation onset at depth {ctx_window + ctx_segment}"
                    )
