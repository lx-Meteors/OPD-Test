...
run_opd() {
    ...
    export LOSS_POSITION_START="${LOSS_POSITION_START:-0}"
    export LOSS_POSITION_END="${LOSS_POSITION_END:-null}"
    ...
    echo "Actor loss response positions: [${LOSS_POSITION_START}, ${LOSS_POSITION_END})"
    ...
    local -a cmd=(
        ...
        "++actor_rollout_ref.actor.loss_position_start=${LOSS_POSITION_START}"
        "++actor_rollout_ref.actor.loss_position_end=${LOSS_POSITION_END}"
