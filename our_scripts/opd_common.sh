...
REPO_ROOT="$(cd "${OUR_SCRIPTS_DIR}/.." && pwd)"
...
setup_tracking() {
    export WANDB_API_KEY="${WANDB_API_KEY:-004ba186f7e1f9bd08fe620ddeaaf98ef356c95f}"
...
run_opd() {
    ...
    export TEST_DATASET="${TEST_DATASET:-[\"datasets/test_data/AIME25/test.parquet\",\"datasets/test_data/AMC23/test.parquet\",\"datasets/test_data/AIME24/test.parquet\"]}"
    ...
    require_path "${REPO_ROOT}/${TRAIN_DATASET}"
    require_path "${REPO_ROOT}/datasets/test_data/AIME25/test.parquet"
    require_path "${REPO_ROOT}/datasets/test_data/AMC23/test.parquet"
    require_path "${REPO_ROOT}/datasets/test_data/AIME24/test.parquet"
    ...
    experiment_name="${combo_name}_${ADV_ESTIMATOR}_${actor_model_name}_${reward_model_name}_${MAX_RESP_LENGTH}-T_${TEMPERATURE}-Tch_${TEACHER_TEMPERATURE}-n_${N_RESPONSES}-mbs_${MINI_BATCH_SIZE}-topk_${LOG_PROB_TOP_K}-topk_strategy_${TOP_K_STRATEGY}-rw_${REWARD_WEIGHT_MODE}-tchwin_${TEACHER_CTX_WINDOW}-${timestamp}"
    ...
    export WANDB_RUN_GROUP="${WANDB_RUN_GROUP:-${combo_name}}"
